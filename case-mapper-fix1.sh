#!/usr/bin/env bash
set -euo pipefail
echo ">> Applying Case Mapper fix: skip pure-number tokens in extractTerms (500,000 -> no longer yields "500"/"000" as fake legal issues)"
mkdir -p "$(dirname "lib/case-mapper/analyze.ts")"
cat > "lib/case-mapper/analyze.ts" << 'CMFIX_EOF'
...
git add -A
git commit -m "Fix Case Mapper: exclude pure-numeric tokens from extracted legal-issue terms"
git push
